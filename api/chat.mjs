export default async function handler(req,res){
 ...
 const data=await response.json();const raw=data.choices?.[0]?.message?.content||'';const reply=raw.replace(/<think>[\s\S]*?<\/think>/gi,'').trim()||'I could not produce a reply.';return res.status(200).json({reply});
}
