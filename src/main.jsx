...
function OutreachWorkspace() {
  ...
  const current = items.find((x) => x.id === selected) || items[0];
  const update = (field, value) => setItems((all) => { const next = all.map((x) => x.id === selected ? { ...x, [field]: value } : x); localStorage.setItem("fixaur-outreach", JSON.stringify(next)); return next; });
