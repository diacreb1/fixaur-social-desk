...
for (const [name, file, count, fields] of selected) {
  ...
  for (const [index, item] of items.entries()) {
    if (item.date !== date) throw new Error(`${name}[${index}]: stale date ${item.date || "missing"}; expected ${date}`);
    if (item.id && !item.id.startsWith(`${date}-`)) throw new Error(`${name}[${index}]: stale id ${item.id}`);
